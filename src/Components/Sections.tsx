...
export default function Sections({ list, callback }: Props) {
    ...
    return <Card type={1}>
        <div ref={mainContainer} className="flex autoScroll" style={{ position: "relative" }}>
            {list.map(item => <span className="pointer" key={`ChartAnimate-PanelSelection-${item.id}`} onClick={(e) => { updateLine(e.target as HTMLElement); callback(item.id) }}><Card type={0}><span style={{ whiteSpace: "nowrap" }}>{item.displayedName}</span></Card></span>)}
